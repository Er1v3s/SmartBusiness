...
import type { GetServiceByParamsQuery, Service } from "../../../models";
...
export const ServicesSection = () => {
  ...
  // Fetch services from ServiceContext
  const fetchServicesFromContext = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const params: GetServiceByParamsQuery = {};
      ...
      setError(error?.detail || "Błąd podczas pobierania usług");
    } finally {
      setLoading(false);
    }
  ...
  // --- Add Service Form State & Validation ---
  type AddForm = {
    name: string;
    description: string;
    category: string;
  ...
  const validateName = useCallback((name: string): string | undefined => {
    if (!name) return "Nazwa usługi jest wymagana.";
    if (name.length < 3) return "Nazwa usługi musi mieć co najmniej 3 znaki.";
    if (name.length > 100)
      return "Nazwa usługi nie może być dłuższa niż 100 znaków.";
    return undefined;
  }, []);
  ...
  const validateVat = useCallback((vat: string): string | undefined => {
  ...
  const validateDuration = useCallback(
    (duration: string): string | undefined => {
      if (!duration) return "Czas trwania jest wymagany.";
      if (!/^\d+$/.test(duration))
        return "Czas trwania musi być liczbą całkowitą.";
      const num = Number(duration);
      if (num < 1) return "Czas trwania musi być większy niż 0.";
      if (num > 600) return "Czas trwania nie może być dłuższy niż 600 minut.";
      return undefined;
    },
    [],
  );
  const validateAddForm = useCallback(
    (form: AddForm): AddFormErrors => {
      return {
  ...
  const handleAdd = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await fetchServicesFromContext();
      setAddForm({
        name: "",
        description: "",
        category: "",
      ...
      showAlert({
        ...
        message: error?.detail || "Błąd podczas dodawania usługi",
        type: "error",
        duration: 3000,
      });
    } finally {
  ...
  // Modal component for editing service
  function EditServiceModal({
    ...
    onSave: (form: EditServiceForm) => Promise<boolean>;
    onDelete: () => Promise<boolean>;
  }) {
    const [localForm, setLocalForm] = useState<EditServiceForm | null>(form);
    ...
    const [editFormTouched, setEditFormTouched] = useState<{
      [K in keyof EditServiceForm]?: boolean;
    }>({});
    const validateEditForm = useCallback(
      (form: EditServiceForm): EditFormErrors => {
        return {
          name: validateName(form.name),
          description: validateDescription(form.description),
          category: validateCategory(form.category),
      ...
      >
        ...
        >
          ...
          <label htmlFor="edit-duration">
            ...
            <input
              id="edit-duration"
              className={`w-full rounded border-2 px-4 py-2 shadow-sm sm:text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white ${editFormErrors.duration && editFormTouched.duration ? "border-red-500 dark:border-red-400" : "border-gray-300"}`}
              type="number"
              min="1"
              step="1"
              value={localForm.duration}
              onChange={(e) => {
                setLocalForm({ ...localForm, duration: e.target.value });
                setEditFormTouched((t) => ({ ...t, duration: true }));
              }}
              onBlur={() =>
                setEditFormTouched((t) => ({ ...t, duration: true }))
              }
            ...
            />
            {editFormErrors.duration && editFormTouched.duration && (
              <span className="mt-1 text-xs text-red-500">
          ...
          <label htmlFor="edit-vat">
            ...
            <input
              id="edit-vat"
              className={`w-full rounded border-2 px-4 py-2 shadow-sm sm:text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white ${editFormErrors.vat && editFormTouched.vat ? "border-red-500 dark:border-red-400" : "border-gray-300"}`}
              type="number"
              min="0"
              step="1"
              value={localForm.vat}
              onChange={(e) => {
                setLocalForm({ ...localForm, vat: e.target.value });
                setEditFormTouched((t) => ({ ...t, vat: true }));
              }}
              onBlur={() => setEditFormTouched((t) => ({ ...t, vat: true }))}
              placeholder="VAT"
            />
            {editFormErrors.vat && editFormTouched.vat && (
              <span className="mt-1 text-xs text-red-500">
                {editFormErrors.vat}
              </span>
            )}
    ...
    <div className="h-full w-full p-4">
      ...
      >
        ...
        <div className="col-span-1 flex flex-col sm:col-span-1">
          <input
            ...
            onChange={(e) => {
              ...
              setAddFormTouched((t) => ({ ...t, duration: true }));
            }}
            onBlur={() => setAddFormTouched((t) => ({ ...t, duration: true }))}
            required
            type="number"
            min="1"
            step="1"
            placeholder="Czas"
            className={`input rounded-lg border-2 px-3 py-2 text-gray-800 dark:text-gray-100 ${addFormErrors.duration && addFormTouched.duration ? "border-red-500 dark:border-red-400" : "border-gray-200 dark:border-gray-700"}`}
          />
          {addFormErrors.duration && addFormTouched.duration && (
            <span className="mt-1 text-xs text-red-500">
              {addFormErrors.duration}
            </span>
          )}
        </div>
        <div className="col-span-1 flex flex-col sm:col-span-1">
        ...
        <div className="col-span-1 flex justify-center sm:col-span-8">
          <ButtonSuccess
            text={addFormSubmitting ? "Dodawanie..." : "Dodaj usługę"}
            type="submit"
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-right">{s.tax}%</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <EditServiceModal
        ...
        onSave={async (localForm) => {
          if (!localForm || !editingId) return false;
          const price = parseFloat(localForm.price);
          const tax = parseInt(localForm.vat, 10);
          const duration = parseInt(localForm.duration, 10);
          if (isNaN(price) || isNaN(tax) || isNaN(duration)) {
            showAlert({
              title: "Błąd danych",
              message: "Cena, VAT i czas trwania muszą być liczbami.",
              type: "error",
              duration: 3000,
            });
            return false;
          ...
          try {
            ...
            showAlert({
              title: error?.title || "Błąd",
              message: error?.detail || "Błąd podczas aktualizacji usługi",
              type: "error",
              duration: 3000,
            });
            return false;
        ...
        onDelete={async () => {
          ...
          try {
            ...
            showAlert({
              title: error?.title || "Błąd",
              message: error?.detail || "Błąd podczas usuwania usługi",
              type: "error",
              duration: 3000,
            });
            return false;
